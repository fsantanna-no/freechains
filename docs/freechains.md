...
- Nodes
    - `Send_10_Nodes`: `message_10:412`
        -  `[535]` `toask` list of nodes to ask if server already have
        -  `[536]` `togo` list of missing nodes to send to server
        -  `[541]` iterates over head nodes to transmit
            - `[559]` no more nodes, sends `0`
...
-
    -
        -
            - `[570]` awaits response (0=need, 1=have)
                - `[575]` need: saves in `togo` and ask about children
            - `[594]` more to ask, sends `0` and next hash, repeats step above
...
-
    -
        -
            - `[591]` no more to ask, starts sending `togo`
            - `[420]` `Go`
            - `[424]` Iterates over `togo`
                - sends backwards, starts from oldest / closest to genesis
                - `[442]` no more to send, sends `0`
                - `[450]` TODO
